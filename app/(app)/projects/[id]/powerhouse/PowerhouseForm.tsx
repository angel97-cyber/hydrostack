...
<div className="mx-auto max-w-6xl px-6 py-8">
      ...
      {/* §4 Generator & Powerhouse */}
      <Section marker="4" title="Generator & Powerhouse Layout" subtitle="AHEC §12 · Reference MH Standard 2014 §2.3.1.7">
        <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          <NumField label="Tailwater min elev." unit="m a.s.l." value={inputs.tailwaterMinElevationMasl}
            step={0.5} onChange={(n) => update('tailwaterMinElevationMasl', n)} />
          <NumField label="Design flood level (HFL)" unit="m a.s.l." value={inputs.designFloodLevelMasl}
            step={0.1} onChange={(n) => update('designFloodLevelMasl', n)} hint="100-yr (≥30 kW) or 50-yr (<30 kW)" />
          <NumField label="Site elevation" unit="m a.s.l." value={inputs.siteElevationMasl}
  step={1} onChange={(n) => update('siteElevationMasl', n)} />
<NumField label="Water temperature" unit="°C" value={inputs.waterTemperatureC}
  step={1} min={0} max={40} onChange={(n) => update('waterTemperatureC', n)}
